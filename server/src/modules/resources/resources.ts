...
export function getData(dirname?: string) {
    ...
    try {
        ...
        for (let dirname of dirnames) {
            ...
            const data = getFilesData(dirname as EnumResourceDirectory);
...
export function unlink(filespath) {
    ...
    // Delete files
    try {
        const count = deleteFiles(filespath);
...
export function rename(currentName: string, newName: string, dirpath: string): boolean {
