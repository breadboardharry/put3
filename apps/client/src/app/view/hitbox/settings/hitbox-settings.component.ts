...
export class HitboxSettingsComponent implements OnInit {
    ...
    constructor(public resourceService: MediaService) {}

    ngOnInit(): void {
        this.resourceService
            .getDataByType(EnumResourceType.AUDIO)
            .then((data) => {
                this.tracks = data;
            });
    }
    ...
    isDisabled(trigger: Trigger): boolean {
        ...
        for (let action of Object.values(this.hitbox.events) as Action[]) {
            ...
            if (
                'lockedTriggers' in action &&
                action.lockedTriggers &&
    ...
    isActionEnabled(action: Action, trigger: Trigger): boolean {
        return (
            action.triggers.includes(ETrigger.All) ||
            action.triggers.includes(trigger.value)
        );
    }
}
