...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    ...
    else if (uploadFolder.audio.includes(fileExt)) uploadPath = 'public/audio';
    else
...
function filename(file, config = 'none') {
  let filename;
  ...
  switch (config) {
    ...
    case 'desktop':
      filename = 'desktop' + path.extname(file.originalname).toLowerCase();
      break;
